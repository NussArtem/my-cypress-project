...
async function runAutoclickerSteps(page, userData, checkStop = null) {
  ...
  try {
    ...
    const hasExpiredSession = sessionExpiredTexts.some(
      text =>
    ...
    const hasCitasPageElements = await page
      .evaluate(() => {
        ...
        const citasDisponiblesText =
          document.body.innerText.includes(
            'Selecciona una de las siguientes citas disponibles',
          ) ||
        ...
        const citaBlocks = Array.from(document.querySelectorAll('*')).filter(
          el => {
            const text = el.textContent || '';
            return (
              /CITA\s*\d+/i.test(text) &&
              (text.includes('Día:') ||
                text.includes('Hora:') ||
                text.includes('/202'))
            );
        ...
        return {
          ...
          hasCitas:
            citasDisponiblesText ||
            citaBlocks.length > 0 ||
            (radioButtons.length > 0 && hasDatePattern && hasTimePattern),
        };
      })
      .catch(() => ({
    ...
    const hasCitasAvailable =
      !hasNoCitas && hasCitasPageElements.hasCitas && !hasExpiredSession;
    ...
    if (hasNoCitas || !hasCitasAvailable) {
      ...
      if (hasNoCitas) {
        ...
        console.log(
          '   ❌ Элементы страницы с доступными записями НЕ найдены!',
        );
        console.log(
          `   ❌ Текст "citas disponibles": ${hasCitasPageElements.hasCitasDisponiblesText ? 'ДА' : 'НЕТ'}`,
        );
        console.log(
          `   ❌ Блоки с записями (CITA): ${hasCitasPageElements.citaBlocksCount}`,
        );
        console.log(
          `   ❌ Радио-кнопки: ${hasCitasPageElements.radioButtonsCount}`,
        );
      }
      console.log('='.repeat(60));
      return { success: false, hasCitas: false };
      ...
      if (checkStop && checkStop()) {
        console.log(
          '🛑 Получен запрос на остановку автокликера (перед отправкой уведомлений)',
        );
