...
async function runAutoclicker(
  ...
  try {
    ...
    try {
      ...
      const launchMode = isProduction
        ? 'headless (production)'
        : 'видимый режим (локально)';
      console.log(
        `🚀 Chrome с remote debugging не найден, запускаю Chrome (${launchMode})...`,
      );
      ...
      // Используем системный Chrome для локальной разработки, встроенный Chrome для production
      let executablePath = null;

      // Только для локальной разработки используем системный Chrome
      if (!isProduction) {
        if (platform === 'darwin') {
          executablePath =
      ...
      // Для production (Railway) не указываем executablePath - используем встроенный Chrome от Puppeteer

      const launchOptions = {
        headless: process.env.HEADLESS === 'true', // Только если явно указано HEADLESS=true
      ...
      // Добавляем executablePath только для локальной разработки
      if (executablePath && !isProduction) {
        launchOptions.executablePath = executablePath;
      }

      // Добавляем use-mock-keychain только для macOS
      if (platform === 'darwin' && !isProduction) {
        launchOptions.args.push('--use-mock-keychain');
      }

      browser = await puppeteer.launch(launchOptions);
      console.log(`✅ Chrome запущен (${launchMode})`);
